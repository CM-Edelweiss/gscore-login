...
# 当前皎皎角短信验证码是 4 位；宽限到 4-8 位，真伪由服务端判
...
async def dna_start(payload: StartPayload) -> StartResponse:
...
async def dna_send_sms(payload: SendSmsPayload) -> JSONResponse:
    ...
    return _result(await send_sms(session, payload.mobile, payload.v_json))


@router.post("/login")
async def dna_login(payload: LoginPayload) -> JSONResponse:
    if not _MOBILE_RE.match(payload.mobile):
        return _result(LoginResultModel(ok=False, msg="手机号格式错误"))
    ...
    if session.status == "success":
        # 已登录的会话不重复跑皎皎角接口；DNAUID 端拿过结果再来访问就让它走 done 页
